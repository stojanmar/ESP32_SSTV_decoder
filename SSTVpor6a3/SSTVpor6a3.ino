...
void setupI2S() {
  ...
  i2s_config_t cfg = {
    ...
    //.sample_rate = 46300, //46400, //Tune this for best color alignment
...
void decodeLine(int lineNum) {
  //First skip one porch
  float f = sampleAverageFreq(128);  //this works dummy just to skip 128 samples
  // Green channel
  for (int x = 0; x < PIXELS_PER_LINE; x++) {
    float f = sampleAverageFreq(SAMPLES_PER_PIXEL);
    lineG[x] = (uint8_t)mapFreqToColor(f);
  }
   f = sampleAverageFreq(256);  //this works dummy just to skip 256 samples
  // Blue channel
  for (int x = 0; x < PIXELS_PER_LINE; x++) {
    float f = sampleAverageFreq(SAMPLES_PER_PIXEL);
...
void loop() {
  ...
  float f = sampleAverageFreq(30);  // smaller block for better timing
